...
/// Markdown, HTML, PDF, JSON, PlainText, CSV, DOCX, and PPTX formats.
/// </summary>
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            switch (options.Format)
            {
                ...
                {
                    await BuildDocxAsync(conversation, options, title, outputPath, ct);
                    break;
                }
                case ExportFormat.Pptx:
                {
                    await BuildPptxAsync(conversation, options, title, outputPath, ct);
                    break;
                }
                default:
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            switch (options.Format)
            {
                ...
                case ExportFormat.Docx:
                {
                    await BuildBatchDocxAsync(conversations, options, title, outputPath, ct);
                    break;
                }
                default:
                    return ExportResult.Fail($"Unsupported export format: {options.Format}");
            ...
            switch (options.Format)
            {
                ...
                case ExportFormat.Pptx:
                {
                    return ExportResult.Fail(
                        $"Search results export is not yet supported for {options.Format} format.");
                }
                default:
                    return ExportResult.Fail($"Unsupported export format: {options.Format}");
            }
...
";
    ...
    /// <summary>
    /// Exports a single conversation as a Word document (.docx).
    /// </summary>
    private async Task BuildDocxAsync(
        ConversationEntity conversation,
        ExportOptions options,
        string title,
        string outputPath,
    ...
    /// <summary>
    /// Exports multiple conversations as a single Word document (.docx)
    /// with page breaks between conversations.
    /// </summary>
    private async Task BuildBatchDocxAsync(
        IReadOnlyList<ConversationEntity> conversations,
        ExportOptions options,
        string title,
        string outputPath,
    ...
    /// Includes title, metadata, messages, citations, and a footer — matching the
    /// visual structure of Markdown/HTML/PDF exports.
    /// </summary>
    ...
    // ════════════════════════════════════════════════════════════════
    //  PPTX generation (OpenXML SDK)
    // ════════════════════════════════════════════════════════════════

    /// <summary>
    /// Exports a conversation as a PowerPoint presentation (.pptx).
    /// Slide 1 is a title slide; subsequent slides present each user/assistant
    /// exchange with the user question as a heading and the assistant response as body text.
    /// </summary>
    private async Task BuildPptxAsync(
        ConversationEntity conversation,
        ExportOptions options,
        string title,
        string outputPath,
